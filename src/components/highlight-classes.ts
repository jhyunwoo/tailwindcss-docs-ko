...
type HastContentNode = HastElementNode | HastTextNode | HastUnknownNode;
...
type HastElementNode = {
  ...
  children: HastContentNode[];
};

type HastUnknownNode = {
  type: string;
  tagName?: string;
  value?: string;
  properties?: Record<string, unknown>;
...
type ShikiHastElement = Parameters<ShikiTransformerContext["addClassToHast"]>[0];
...
export function highlightClasses(opts: HighlightClassesOptions): ShikiTransformer {
  return {
    ...
    code(ast) {
      let lines = ast.children.filter(isElementNode) as HastElementNode[];

      // Lines that should be removed from the output
      let toRemove = new Set<HastContentNode>();
...
function highlightClassesIn(
...
): HastElementNode[] | null {
  let textNode = el.children[0];
  if (!isTextNode(textNode)) return null;

  function create(value: string): HastElementNode {
    return {
      ...el,
  ...
  for (let [start, end] of matches) {
    ...
    this.addClassToHast(classEl as ShikiHastElement, opts.highlightedClassName);
...
function parseHighlightDirective(el: HastElementNode): Highlight | null {
...
function getControlComment(el: HastElementNode): string | null {
...
function isTextNode(node: HastContentNode | undefined): node is HastTextNode {
  return !!node && node.type === "text" && typeof node.value === "string";
}

function isElementNode(node: unknown): node is HastElementNode {
  return (
    !!node &&
    typeof node === "object" &&
    (node as HastUnknownNode).type === "element" &&
    typeof (node as HastUnknownNode).tagName === "string" &&
    Array.isArray((node as HastUnknownNode).children)
  );
}

export function getTextContent(element: HastContentNode): string {
  if (isTextNode(element)) {
    return element.value;
  }

  if (isElementNode(element) && (element.tagName === "div" || element.tagName === "span")) {
